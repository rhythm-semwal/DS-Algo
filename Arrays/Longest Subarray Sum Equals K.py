...
class Solution:
    def subarraySum(self, nums: List[int], k: int) -> int:
        count = 0
        # At the beginning the sum of array is 0 as we have not considered any element yet,
        # So sum is 0 and there is 1 subarray (empty array) with 0 sum.
        hash_map = {0: 1}
        ...
        for i in range(len(nums)):
            ...
            # prefix[i] - prefix[j] = k where i > j
            # prefix[i] - k = prefix[j]
            if cum_sum - k in hash_map:
                count += hash_map[cum_sum-k]

            if cum_sum not in hash_map:
                hash_map[cum_sum] = 1
            else:
                hash_map[cum_sum] += 1
